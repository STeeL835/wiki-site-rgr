...
namespace WikiSite.PL.ASP.Classes
{
	public class DefaultRoleProvider : RoleProvider
	{
		...
		{
			return RoleVM.GetRoles().Select(role => role.Name).ToArray();
		}
		...
		/// <param name="id">The user to return a list of roles for.</param>
		public override string[] GetRolesForUser(string id)
		{
			return new[] {RoleVM.GetRole( UserVM.GetUser(Guid.Parse(id)).RoleId ).Name};
		}
